...
void TTigFragment::Print()	{
  ...
  printf("\tChannel Address: 0x%08x",ChannelAddress); printf("\t\tChannel Num:      %i\n",ChannelNumber);  
  printf("\tCharge		   0x%08x",Charge);			printf("\t\tEnergy:		      %f\n",ChargeCal);  
  ...
  if(!wavebuffer.empty()) printf("Has a wave form stored.\n");
  else printf("Does Not have a wave form stored.\n");
}


void TTigFragment::SetTimeStamp()	{
	TimeStamp = (((int64_t) TimeStampHigh)<<24) | (TimeStampLow & 0x00ffffff); 
}

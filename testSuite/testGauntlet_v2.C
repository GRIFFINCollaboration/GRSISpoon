...
void testGauntlet()	{
	...
	printf("\t" BLUE  "standard  file opened: %s" RESET_COLOR "\n", standard->GetName());
	TFile *contender = new TFile("fragment27422_000.root");
	printf("\t" RED   "contender file opened: %s" RESET_COLOR "\n", contender->GetName());
	...
	if(nentries_ct != nentries_st)	{
		printf("warning standrad  tree has " BLUE "%i fragments!" RESET_COLOR "\n"  ,nentries_st;);
		printf("warning contender tree has " RED  "%i fragments!" RESET_COLOR "\n"  ,nentries_ct;);
		return;
	}
